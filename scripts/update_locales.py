...
Usage: python3 scripts/update_locales.py <new-tags.tsv>

The TSV is `tag<TAB>English description`, one per line, collected from
ExifTool's own output for the tags this port newly emits. Per-language
translations come from that ExifTool release's Lang/*.pm via lang_extract.pl.
Tags ExifTool does not translate keep the English fallback, matching how the
existing files already carry English for untranslated tags. Nothing is
invented.

The input is named on the command line rather than pinned in here: the 13.59
list this was first written for is merged and gone, and the next bump brings
its own.
"""
import subprocess
import sys
...
def main():
    if len(sys.argv) != 2:
        sys.exit(f"usage: {sys.argv[0]} <new-tags.tsv>")
    tsv = Path(sys.argv[1])
    new = load_new(tsv)
    names = list(new.keys())
    names_file = tsv.with_suffix(".names")
